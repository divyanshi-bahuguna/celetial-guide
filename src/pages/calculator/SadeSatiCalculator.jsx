...
const SadeSatiCalculator = () => {
  ...
  const onSubmit = async ({ dob }) => {
    ...
    setResult("");
    try {
      const res = await fetch("http://localhost:5000/api/sade-sati-calculator", {
        method: "POST",
      ...
      console.error(err);
      setResult("Error calculating Sade Sati.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <CalculatorTemplate
      ...
      description="Analyze Sade Sati effects (AI-powered)."
      fields={fields}
      onSubmit={onSubmit}
      loading={loading}
      result={result}
